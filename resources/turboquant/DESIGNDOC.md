...
When $b = 2$ (our operating point), $Q_{\text{prod}}$ reduces the MSE stage to 1-bit quantization. The paper's Theorem 1 gives $D_{\text{mse}}(b=1) \approx 0.36$ versus $D_{\text{mse}}(b=2) \approx 0.117$, a 3× increase in reconstruction error.
While the QJL stage removes the multiplicative bias, the inner product (IP) distortion bound for $Q_{\text{prod}}(b=2)$ from Theorem 2 is $\approx 0.56/d$ (= pure variance, since $Q_{\text{prod}}$ is unbiased). By contrast, $Q_{\text{mse}}(b=2)$ spends all $b=2$ bits on MSE-optimal coding. Its IP estimation MSE can be derived as follows: each rotated coordinate has scalar MSE cost $D_{\text{mse}}(b=2) \approx 0.117$ (relative to unit-variance input), so the per-coordinate quantization error is $E[e_j^2] = D_{\text{mse}}/d$ (since each coordinate has variance $1/d$ under JL rotation of a unit vector). Since coordinate-wise quantization errors are independent, the IP estimation MSE is $\sum_j w_j^2 \cdot E[e_j^2] = (D_{\text{mse}}/d) \cdot \|z\|^2 = D_{\text{mse}}/d \approx 0.117/d$. Note that this is the total MSE (= Bias² + Variance), not pure variance; the actual variance is strictly smaller. Comparing Q_mse's MSE upper bound against Q_prod's variance is conservative (unfavorable to Q_mse), yet Q_mse is still approximately 5× better ($0.117/d$ vs $0.56/d$). The net effect is that $Q_{\text{prod}}$ trades a $\geq$ 5× increase in IP estimation error for bias removal, which is unfavorable at $b=2$.

**Bias in Q_mse acts conservatively for HNSW pruning.**
The MSE quantizer's bias attenuates the estimated inner product ($\hat{\cos\theta} \approx \alpha \cos\theta$, $\alpha < 1$). In the L2 distance formula $\hat{d} = \|v\|^2 + \|\delta\|^2 - 2\|v\|\|\delta\|\hat{\cos\theta}$, this causes distance *overestimation*. For candidate pruning, a systematic multiplicative bias preserves the relative ordering of candidates (since the attenuation is monotonic), so the ranking among candidates remains correct and recall is maintained. To compensate for the uniformly inflated distances, `ef_search` may need a modest increase, slightly reducing QPS. An unbiased estimator with higher variance, by contrast, can arbitrarily scramble the ranking — under-estimation of a poor candidate's distance can promote it above a good one, directly harming recall.
...
PolarQuant (Han et al., arXiv:2502.02617, 2025) applies a random preconditioning rotation, then converts each vector into polar coordinates via a recursive transformation (Definition 1 in their paper). The resulting $d - 1$ angles are quantized independently per level, and the norm $\|x\|_2$ is stored exactly. Decoding reconstructs the Cartesian vector by recursively expanding $r_j^{(\ell)} \cdot \cos\hat{\theta}$ and $r_j^{(\ell)} \cdot \sin\hat{\theta}$ through $\log_2 d$ levels, followed by multiplication with $S^\top$ (the transpose of the preconditioning matrix). Theorem 1 in their paper proves $E[\|x - x'\|_2^2] = \varepsilon \cdot \|x\|_2^2$ using $O(\log 1/\varepsilon)$ bits per coordinate.

**Decode structure is incompatible with linear inner product accumulation.**
Our pipeline estimates $\langle v, \delta \rangle$ by the linear accumulation $\sum_j v_j \cdot \hat{q}_j$, where $\hat{q}_j$ are Lloyd-Max centroids looked up from 2-bit codes. This requires no vector reconstruction: the quantized codes directly yield the summands. PolarQuant's decode, by contrast, requires the full recursive polar-to-Cartesian expansion ($\log_2 d$ levels of $\cos/\sin$ multiplications) plus a matrix multiplication with $S^\top$ (the inverse of the preconditioning rotation) to recover the Cartesian vector before any inner product can be computed. There is no way to extract a per-coordinate dequantized value from the angle codes without running the complete reconstruction, because each Cartesian coordinate depends on the product of angles across all levels.

**Decode computation cost is prohibitive.**
Regardless of the storage question, each candidate evaluation at scan time would require $O(m)$ trigonometric operations ($\cos/\sin$ per angle) for the polar-to-Cartesian expansion before the inner product can be computed. By contrast, TurboQuant's decode is a single table lookup per code followed by a dot product with the pre-rotated query, involving no transcendental functions.
...
All encode/decode paths use $m$ = min($d$, `HNSW_TQ_MAX_ROT_DIMS`) as the effective number of rotated coordinates, and the decode divisor is $m / \sqrt{N}$ (not $d / \sqrt{N}$).
...
* **Calculation Logic:** `sizeof(float4) + ceil((m * 2.0) / 8.0)` where $m$ = min($d$, `HNSW_TQ_MAX_ROT_DIMS`).
...
* **`void TurboQuantProject(const float *vec, int dim, float scale, uint8 *out_codes)`**
    ...
    * **Logic:**
        1. Computes $m$ = min($d$, `HNSW_TQ_MAX_ROT_DIMS`).
